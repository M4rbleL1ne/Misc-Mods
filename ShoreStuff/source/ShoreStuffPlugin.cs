...
[BepInPlugin("lb-fgf-m4r-ik.shore-stuff", nameof(ShoreStuff), "1.0.1")]
sealed class ShoreStuffPlugin : BaseUnityPlugin
{
    const string K_FR = "FR";
    ...
    static float On_RoomRain_get_FloodLevel(Func<RoomRain, float> orig, RoomRain self) => self.room is Room rm && rm.world?.region?.name is K_FR && rm.waterFlux is Room.WaterFluxController c ? c.fluxWaterLevel : orig(self);
    ...
    static void On_Water_DrawSprites(On.Water.orig_DrawSprites orig, Water self, RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam, float timeStacker, Vector2 camPos)
    {
        ...
        if (!rCam.voidSeaMode && !self.slatedForDeletetion && rCam.room is Room room && room == self.room && room.abstractRoom?.name is string s && string.Equals(s, "FR_C20", StringComparison.OrdinalIgnoreCase))
            sLeaser.sprites[0].shader = rCam.currentCameraPosition == 2 || (ModManager.MSC && room.roomSettings?.DangerType == MoreSlugcatsEnums.RoomRainDangerType.Blizzard) ? Custom.rainWorld.Shaders["WaterSlush"] : Custom.rainWorld.Shaders["WaterSurface"];
    }
