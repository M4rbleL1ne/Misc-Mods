...
[BepInPlugin("lb-fgf-m4r-ik.pa-stuff", nameof(PAStuff), "10.0.0")]
public sealed class PAStuffPlugin : BaseUnityPlugin
{
    public void OnEnable()
    {
        On.SSOracleSwarmer.DrawSprites += (orig, self, sLeaser, rCam, timeStacker, camPos) =>
        {
            ...
            if (self.room?.world?.region?.name == "PA" && rCam.currentPalette.darkness > 0f)
            {
                var light = sLeaser.sprites[0];
                light.shader = Custom.rainWorld.Shaders["LightSource"];
                light.scale = 7.5f;
        ...
        On.RoofTopView.Building.InitiateSprites += (orig, self, sLeaser, rCam) =>
        {
            ...
            if (self.room?.world?.region?.name == "PA")
