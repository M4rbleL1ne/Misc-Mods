...
[BepInPlugin("lb-fgf-m4r-ik.bat-bombs", nameof(BatBombs), "10.0.0")]
public sealed class BatBombsPlugin : BaseUnityPlugin
{
    ...
    public void OnEnable()
    {
        ...
        IL.MultiplayerUnlocks.ctor += il =>
        {
                ...
                x => x.MatchLdarg(1)))
            {
                c.Emit(OpCodes.Ldsfld, il.Import(typeof(SandboxUnlockID).GetField(nameof(SandboxUnlockID.BatBomb))))
                 .Emit(OpCodes.Pop)
                 .Emit(OpCodes.Ldsfld, il.Import(typeof(AbstractObjectType).GetField(nameof(AbstractObjectType.BatBomb))))
                 .Emit(OpCodes.Pop);
            }
            else
        ...
        On.ScavengerBomb.HitSomething += (orig, self, result, eu) =>
        {
            var res = orig(self, result, eu);
            if (self is BatBomb && res && self.room is Room rm && rm.game is RainWorldGame g)
            {
                if (result.obj is PhysicalObject p && p is not Fly and not BatBomb && p.bodyChunks is BodyChunk[] bs)
                {
                    for (var i = 0; i < bs.Length; i++)
                    {
                        var crit = new AbstractCreature(rm.world, StaticWorld.GetCreatureTemplate(CreatureTemplate.Type.Fly), null, rm.GetWorldCoordinate(bs[i].pos), g.GetNewID());
        ...
        On.ScavengerBomb.Explode += (orig, self, hitChunk) =>
        {
            if (self is BatBomb && !self.slatedForDeletetion && self.room is Room rm && rm.game is RainWorldGame g)
            {
                ...
                var pbjs = self.abstractPhysicalObject.stuckObjects;
                for (var m = 0; m < pbjs.Count; m++)
                    pbjs[m].Deactivate();
                rm.PlaySound(SoundID.Token_Collect, vector, 1.2f, 1.5f, self.abstractPhysicalObject);
                self.smoke?.Destroy();
                self.Destroy();
                return;
            }
        ...
        IL.ScavengerTreasury.ctor += il =>
        {
            var c = new ILCursor(il);
            if (c.TryGotoNext(MoveType.After,
                x => x.MatchLdsfld<AbstractPhysicalObject.AbstractObjectType>(nameof(AbstractPhysicalObject.AbstractObjectType.ScavengerBomb))))
                c.EmitDelegate((AbstractPhysicalObject.AbstractObjectType tp) => Random.value > .6f ? AbstractObjectType.BatBomb : tp);
            else
                s_logger.LogError("Couldn't ILHook ScavengerTreasury.ctor!");
        };
    }
