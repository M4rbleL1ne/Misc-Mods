...
public sealed class SeerStuffPlugin : BaseUnityPlugin
{
    ...
    /*static void IL_DeathRain_NextDeathRainMode(ILContext il)
    {
            ...
            x => x.MatchCall(out _)))
        {
            c.Emit(OpCodes.Ldarg_0)
             .EmitDelegate((bool flag, GlobalRain.DeathRain self) => flag || self.globalRain.game.StoryCharacter == s_seer);
        }
        else
    ...
    {
        orig();
        if (ModManager.DLCShared && ChallengeTools.creatureSpawns.TryGetValue("Seer", out var dict))
            dict.Add(new()
            {
    ...
    static void IL_OverseerCommunicationModule_ReevaluateConcern(ILContext il)
    {
        ...
        {
            c.Emit(OpCodes.Ldarg_0)
             .EmitDelegate((bool flag, OverseerCommunicationModule self) => flag || self.overseerAI.creature.world.game.StoryCharacter == s_seer);
        }
        else
