...
sealed class HardhatTutorial : UpdatableAndDeletable
{
    ...
    {
        base.Update(eu);
        if (room is Room rm && rm.BeingViewed && rm.game is RainWorldGame game && game.cameras is RoomCamera[] ar && ar.Length > 0 && ar[0]?.hud?.textPrompt is TextPrompt txt && game.rainWorld?.inGameTranslator is InGameTranslator tr && game.Players.Count > 0 && game.Players[0]?.realizedCreature is Player pl)
        {
            pl.AddFood(1);
