...
[BepInPlugin("lb-fgf-m4r-ik.bounce-rects", nameof(BounceRects), "1.0.0")]
    ...
    public void OnEnable()
    {
        ...
        On.Room.Update += (orig, self) =>
        {
            ...
            for (var i = 0; i < phs.Length; i++)
            {
                ...
                {
                    var obj = phsi[j];
                    if (!(obj is Centipede c && !c.Small) && obj is not PoleMimic and not TentaclePlant and not GarbageWorm and not VoidSpawn and not Oracle and not SeedCob and not Vulture and not BigEel and not MoreSlugcats.StowawayBug)
                    {
